...
class Encoder(nn.Module):
    def __init__(self, hidden_size=512, num_layers=2, drop_out=0.2, 
                 embedding_size=256, bidirection=False, model='LSTM'):
        super().__init__()
        ...
        if model == 'LSTM':
            ...
            self.model = nn.GRU(embedding_size, hidden_size, num_layers, 
                                bidirectional=bidirection, batch_first=True)
        elif model == 'RNN':
            self.model = nn.RNN(embedding_size, hidden_size, num_layers, 
                                bidirectional=bidirection, batch_first=True)
        else:
...
class seq2seq(nn.Module):
    ...
    def forward(self, inputs, targets, teacher_force_ratio):
        ...
        output = torch.zeros((targets.shape[0], targets.shape[1], 
                              len(mal_index2word)), device='cuda')
...
def num_word(number, converter):
    ...
    return word
def train_model(epochs=30, hidden_size=512, num_layers=3, 
                encoder_drop_out=0.2, decoder_drop_out=0.2, embedding_size=256,
                bidirection=False, model='LSTM', lr=1e-3, 
                optimizer_='adam', teacher_force_ratio=0.5,log=False):
    # Set hyperparameters
    epochs = epochs
    ...
    encoder = Encoder(hidden_size=hidden_size, num_layers=num_layers, 
                      drop_out=encoder_drop_out, embedding_size=embedding_size,
                      bidirection=bidirection, model=model)
    decoder = Decoder(hidden_size=hidden_size, num_layers=num_layers, 
                      drop_out=decoder_drop_out, embedding_size=embedding_size,
                      bidirection=bidirection, model=model)
...
parser.add_argument("-nl","--num_layers",default=3,choices=[1,2,3],help='Number of layers in the encoder and decoder',type=int)
parser.add_argument('-e_dp','--e_drop_out',default=0.5,choices=[0.2, 0.3, 0.5],help='Dropout probability in the encoder',type=float)
parser.add_argument('-d_dp','--d_drop_out',default=0.5,choices=[0.2, 0.3, 0.5],help='Dropout probability in the decoder',type=float)
parser.add_argument('-es','--embedding_size',default=64,choices=[32, 64, 256, 512],help='Size of the embedding layer',type=int)
...
parser.add_argument("-m","--model",default='LSTM',choices=['LSTM', 'GRU', 'RNN'],help='which model to choose')
parser.add_argument("-opt","--optimizer",default='adam',choices=['sgd','adam'],help="optimizer function")
parser.add_argument("-tr","--teacher_forcing",default=0.5,choices=[0.2, 0.3, 0.4, 0.5],help='Teacher forcing ratio during training',type=float)
